...
class _FrsPageState extends State<FrsPage> {
  ...
  Future<void> _addCourse(dynamic matakuliahId) async {
      final int courseId = matakuliahId is int ? matakuliahId : int.tryParse('$matakuliahId') ?? 0;
                              ...
                              children: [
                                ...
                                Text(
                                  ...
                                  style: TextStyle(
                                    fontWeight: FontWeight.bold,
                                    fontSize: 16,
                                    color: _totalCredits > _maxCredits 
                                        ? Colors.red 
                                        : Colors.black,
                                  ),
                                ),
                          ...
                          ListView.builder(
                            ...
                            itemBuilder: (context, index) {
                              ...
                              final isSelected = _selectedCourses.any(
                                (selected) => selected['mata_kuliah_id'] == course['id']
                              );
                              return _buildAvailableCourseCard(course, isSelected);
                            },
                          ),
                          const SizedBox(height: 24),
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          _infoRow('NRP', studentInfo['nrp'] ?? studentInfo['nim'] ?? user?['nrp'] ?? '-'),
  ...
  Widget _infoRow(String label, String value) {
    ...
    Widget _buildSelectedCourseCard(Map<String, dynamic> course) {
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      const SizedBox(height: 8),
                      _courseInfoItem(Icons.person, 'Dosen: ${course['nama_dosen'] ?? course['dosen'] ?? course['lecturer'] ?? '-'}'),
                      _courseInfoItem(Icons.calendar_today, '${course['jadwal'] ?? course['schedule'] ?? '-'}'),
                      _courseInfoItem(Icons.book, 'SKS: ${course['sks']?.toString() ?? '0'}'),
                    ],
                  ),
                ),
                // Remove button
      ...
      Widget _buildAvailableCourseCard(Map<String, dynamic> course, bool isSelected) {
        // Add debug print to help identify available keys
        print("Available course keys: ${course.keys.toList()}");
        print("Available course data: $course");
        ...
        return Card(
          ...
          child: InkWell(
            onTap: isSelected || _isSubmitting ? null : () => _addCourse(course['id']),
            borderRadius: BorderRadius.circular(12),
            child: Padding(
              padding: const EdgeInsets.all(16),
              child: Row(
                ...
                children: [
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        ...
                        const SizedBox(height: 8),
                        _courseInfoItem(Icons.person, 'Dosen: ${course['dosen'] ?? course['nama_dosen'] ?? course['lecturer'] ?? '-'}'),
                        _courseInfoItem(Icons.calendar_today, '${course['jadwal'] ?? course['schedule'] ?? '-'}'),
                        _courseInfoItem(Icons.book, 'SKS: ${course['sks']?.toString() ?? '0'}'),
                      ],
                    ),
                  ...
                  if (isSelected)
                    ...
                    IconButton(
                      onPressed: _isSubmitting ? null : () => _addCourse(course['id']),
